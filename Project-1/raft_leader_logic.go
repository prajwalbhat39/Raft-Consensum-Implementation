			...
			if err := this.server.SendRPCCallTo(peerId, "RaftNode.AppendEntries", args, &reply); err == nil {
				...
				if this.state == "Leader" && termWhenHeartbeatSent == reply.Term {
					if reply.Success {
					           ...
					           this.write_log("appending entries success")
						...
						for i := this.commitIndex + 1; i < len(this.log); i++ {
							if this.log[i].Term == this.currentTerm {
								...
								if  matchCount*2 > len(this.peersIds)+1{ // TODO  // When should you update commitIndex to i?
									this.commitIndex = i
								}
							}
						}
						...
						if this.commitIndex != oldCommitIndex {
						  ...
						  this.write_log("AppendEntries reply from %d !success: nextIndex := %d", peerId, currentPeer_nextIndex-1)
